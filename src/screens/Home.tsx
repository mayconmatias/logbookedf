...
// ... (Componentes CoachFeedbackBanner e WeeklyTracker mantidos iguais, omitidos para brevidade) ...
        ...
        {days.map((day, index) => {
          ...
          return (
            <View key={index} style={styles.dayWrapper}>
              <View style={[styles.dayCircle, isCompleted && styles.dayCompleted, isToday && !isCompleted && styles.dayToday]}>
...
export default function Home({ navigation }: HomeProps) {
  ...
  // 1. Carrega dados do perfil ao focar na tela
  useFocusEffect(
    useCallback(() => {
  ...
  // 2. Configura o Realtime para o Badge (Executa uma vez na montagem)
  useEffect(() => {
    // Função para recarregar apenas o contador
    ...
    // Chama imediatamente para garantir o estado atual
    refreshBadge();
    ...
    useCallback(() => {
      const loadDashboard = async () => {
        ...
        try {
          ...
          const [plan, session, freq, msg, notifs] = await Promise.all([
  ...
  // [NOVO] Verificação de Status do Plano
