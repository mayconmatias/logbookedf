...
export default function ProgressionShareModal({
  ...
  const handleShare = async () => {
    if (!set || !progression || progression.length === 0) {
      Alert.alert('Aguarde', 'Dados insuficientes para gerar o card.');
      return;
    }
    ...
    try {
      ...
      await Sharing.shareAsync(uri, { mimeType: 'image/png', dialogTitle: 'Compartilhar sua progressão' });
    } catch (e: any) {
