...
BEGIN
  ...
  -- 2. ANÁLISE DE TEMPO DE DESCANSO
  rest_analysis AS (
  ...
  workout_summary AS (
    SELECT
      ...
      -- [CORREÇÃO] Duração em minutos (apenas se < 4 horas para evitar bugs de treino esquecido aberto)
  ...
  last_session_details AS (
    ...
    FROM base_sets
    WHERE workout_date = (SELECT MAX(workout_date) FROM base_sets)
  ),
  ...
  SELECT jsonb_build_object(
    ...
    'last_session_sets', (SELECT sets_json FROM last_session_details) -- [NOVO]
  ) INTO result_json
  FROM status_calc sc;
