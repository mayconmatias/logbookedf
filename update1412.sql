...
DECLARE
  ...
  result_json jsonb;
BEGIN
  -- Define usuário (Aluno ou Próprio)
  ...
  -- 1. Encontra a data do último treino (ignorando o atual se solicitado)
  SELECT
      ...
      'lastPerformance', (
        ...
        FROM (
          ...
          WHERE
            ...
            -- [SEGURANÇA] Garante que pegamos apenas sets desse treino específico
            AND (p_exclude_workout_id IS NULL OR w.id != p_exclude_workout_id)
        ) lp
      ),
      'bestPerformance', (
...
BEGIN
  ...
  raw_sets AS (
    ...
    WHERE
      ...
      AND s.weight >= 0 AND s.reps > 0 -- Considera pesos 0 (corporal) mas exige reps > 0
  ),
  calculated_sets AS (
    SELECT
      *,
      CASE 
        WHEN safe_reps > 0 THEN (safe_weight * power(1 + (0.032 * safe_reps), 0.9))::numeric(10, 2)
        ELSE 0 
      END AS e1rm,
  ...
  SELECT jsonb_build_object(
    ...
    -- [CORREÇÃO DO GRÁFICO DE FORÇA]
    -- O ORDER BY deve ficar DENTRO do json_agg
    'chartDataE1RM', (
      SELECT COALESCE(
        json_agg(
...
CREATE OR REPLACE VIEW "public"."unified_exercise_catalog" AS
 SELECT "ed"."id",
    "ed"."name",
    "ed"."video_url",
    "ed"."default_notes",
...
UNION ALL
 SELECT "ed"."id",
   ...
   FROM (((("public"."exercise_definitions" "ed"
     ...
     JOIN "public"."marketplace_products" "mp" ON (("mp"."linked_library_id" = "el"."id")))
     JOIN "public"."user_purchases" "up" ON (("up"."product_id" = "mp"."id")))
...
CREATE POLICY "Gerenciar Programas (Dono)" ON "public"."programs" USING (("auth"."uid"() = "coach_id")) WITH CHECK (("auth"."uid"() = "coach_id"));
...
CREATE POLICY "Ver Programas" ON "public"."programs" FOR SELECT USING ((("auth"."uid"() = "coach_id") OR ("auth"."uid"() = "student_id")));
