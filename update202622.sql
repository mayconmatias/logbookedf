...
CREATE OR REPLACE FUNCTION "public"."calculate_e1rm"("w" numeric, "r" integer, "rpe" numeric DEFAULT NULL::numeric) RETURNS numeric
...
BEGIN
  ...
  IF w IS NULL OR w <= 0 OR r IS NULL OR r <= 0 THEN
    RETURN 0;
  END IF;

  -- Lógica de Cálculo (Fórmula de Epley)
  e1rm := w * (1 + (r::numeric / 30.0));
...
ALTER FUNCTION "public"."calculate_e1rm"("w" numeric, "r" integer, "rpe" numeric) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."calculate_volume_load"("weight" numeric, "reps" integer) RETURNS numeric
    LANGUAGE "plpgsql" IMMUTABLE
    SET "search_path" TO 'public'
    AS $$
BEGIN
  RETURN COALESCE(weight, 0) * COALESCE(reps, 0);
END;
$$;


ALTER FUNCTION "public"."calculate_volume_load"("weight" numeric, "reps" integer) OWNER TO "postgres";
...
DECLARE
  ...
  v_new_program_id uuid;
  v_new_program jsonb;
  -- CONSTANTE DE LIMITE NO BANCO
  c_free_limit CONSTANT integer := 1; 
BEGIN
  ...
  -- 2. Conta programas autorais existentes
  -- (Ignora templates comprados, conta apenas os criados pelo usuário)
  ...
  -- 3. TRAVA DE SEGURANÇA: Se for free e já tiver 3 ou mais, erro.
  -- A verificação do lado do servidor garante que ninguém burle o app.
  IF (v_plan = 'free' OR v_plan IS NULL) AND v_count >= c_free_limit THEN
    RAISE EXCEPTION 'LIMIT_REACHED: Você atingiu o limite de % programas para o plano gratuito.', c_free_limit;
  END IF;
...
BEGIN
  ...
  calculated_sets AS (
    SELECT
      *,
      -- [CORREÇÃO] Usamos raw_sets.rpe (que já está na seleção acima como 'rpe')
      -- Em vez de 's.rpe', usamos apenas 'rpe' pois estamos selecionando de raw_sets
      public.calculate_e1rm(safe_weight, safe_reps::int, rpe) AS e1rm,
...
CREATE OR REPLACE FUNCTION "public"."get_exercise_analytics_v2"("p_definition_id" "uuid", "p_target_user_id" "uuid" DEFAULT NULL::"uuid") RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
...
BEGIN
  ...
  base_sets AS (
    SELECT 
      s.id, s.weight, s.reps, s.rpe, s.set_type, s.performed_at, s.set_number,
      s.exercise_id, e.workout_id, w.workout_date, w.created_at
    FROM sets s
    JOIN exercises e ON s.exercise_id = e.id
    JOIN workouts w ON e.workout_id = w.id
    WHERE w.user_id = v_user_id 
  ...
  -- 2. TEMPO DE DESCANSO
  rest_analysis AS (
    SELECT avg(rest_seconds) as avg_rest
    FROM (
      SELECT EXTRACT(EPOCH FROM (performed_at - LAG(performed_at) OVER (PARTITION BY exercise_id ORDER BY performed_at))) as rest_seconds
      FROM base_sets
    ) sub
    WHERE rest_seconds BETWEEN 10 AND 600
  ),

  -- 3. TIPO FAVORITO
  fav_type AS (
    SELECT set_type, count(*) as qtd
    FROM base_sets WHERE set_type != 'normal' 
    GROUP BY set_type ORDER BY qtd DESC LIMIT 1
  ),

  -- 4. SUMÁRIO (Mantém dados de hoje aqui para os gráficos, pois queremos ver a evolução atual nos gráficos)
  workout_summary AS (
    SELECT
      workout_date,
      workout_id,
      SUM(weight * reps) as vol_load,
      AVG(reps) as avg_reps,
      COALESCE((EXTRACT(EPOCH FROM (MAX(performed_at) - MIN(performed_at))) / 60)::int, 0) as duration_min,
      MAX(CASE WHEN set_type != 'warmup' THEN public.calculate_e1rm(weight, reps, rpe) ELSE 0 END) as max_e1rm,
      MIN(CASE WHEN set_type != 'warmup' THEN public.calculate_e1rm(weight, reps, rpe) ELSE 0 END) as min_e1rm,
      AVG(CASE WHEN set_type != 'warmup' THEN public.calculate_e1rm(weight, reps, rpe) ELSE 0 END) as avg_e1rm,
      COUNT(*) as total_sets
    FROM base_sets
    GROUP BY workout_date, workout_id
  ),
  ...
  trends AS (
    SELECT
      *,
      LAG(vol_load) OVER (ORDER BY workout_date) as prev_vol,
      LAG(avg_reps) OVER (ORDER BY workout_date) as prev_reps
  ...
  status_calc AS (
    SELECT
      (ARRAY_AGG(
        CASE 
          WHEN vol_load > prev_vol AND avg_reps >= prev_reps THEN 'progressing'
          WHEN vol_load < (prev_vol * 0.9) THEN 'regressing'
          ELSE 'stagnated'
        END 
        ORDER BY workout_date DESC
      ))[1] as current_status,
      
      (SELECT MAX(workout_date) FROM trends WHERE vol_load > prev_vol AND avg_reps >= prev_reps) as last_prog_date,

      AVG(duration_min) FILTER (WHERE duration_min > 0 AND duration_min < 240) as avg_duration_global
    FROM trends
  ),

  -- 7. RECAP DA SESSÃO ANTERIOR (CORREÇÃO APLICADA AQUI)
  last_session_details AS (
    SELECT json_agg(json_build_object(
      ...
      'rpe', rpe,
      'type', set_type
    ) ORDER BY set_number ASC) as sets_json
    FROM base_sets
    -- AQUI: Buscamos a maior data que seja MENOR que hoje.
    -- Isso garante que pegamos o treino passado, ignorando o que está sendo digitado agora.
    WHERE workout_date = (
  ...
  -- 8. GRÁFICOS (Mantemos dados de hoje nos gráficos para o usuário ver o ponto atual subindo)
  chart_e1rm_json AS (
    SELECT COALESCE(json_agg(json_build_object(
        'date', workout_date, 'max', max_e1rm, 'avg', round(avg_e1rm::numeric, 1), 'min', min_e1rm
      ) ORDER BY workout_date ASC), '[]'::json) as data
    FROM (SELECT * FROM trends ORDER BY workout_date DESC LIMIT 30) sub
  ),

  chart_volume_json AS (
    SELECT COALESCE(json_agg(json_build_object(
        'date', workout_date, 'vol_load', vol_load, 'total_reps', (avg_reps * total_sets)::int, 'sets_count', total_sets
      ) ORDER BY workout_date ASC), '[]'::json) as data
    FROM (SELECT * FROM trends ORDER BY workout_date DESC LIMIT 20) sub
  )

  SELECT jsonb_build_object(
    'last_progression', jsonb_build_object(
        'date', sc.last_prog_date,
        'days_ago', COALESCE(CURRENT_DATE - sc.last_prog_date, -1)
    ),
    'current_status', COALESCE(sc.current_status, 'insufficient_data'),
    'avg_duration_min', COALESCE(sc.avg_duration_global::int, 0),
    'avg_rest_sec', COALESCE((SELECT avg_rest::int FROM rest_analysis), 0),
    'favorite_set_type', (SELECT set_type::text FROM fav_type),
    'chart_e1rm', (SELECT data FROM chart_e1rm_json),
    'chart_volume', (SELECT data FROM chart_volume_json),
    'last_session_sets', (SELECT sets_json FROM last_session_details)
  ) INTO result_json
  FROM status_calc sc;
...
ALTER FUNCTION "public"."get_exercise_analytics_v2"("p_definition_id" "uuid", "p_target_user_id" "uuid") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."get_student_diagnostic_feed"("p_student_id" "uuid", "p_limit" integer DEFAULT 20, "p_offset" integer DEFAULT 0, "p_filter_unread" boolean DEFAULT false) RETURNS TABLE("workout_id" "uuid", "workout_name" "text", "workout_date" "date", "is_read" boolean, "total_exercises" bigint, "total_sets" bigint, "duration_seconds" integer, "avg_rest_seconds" integer, "exercises_json" "jsonb")
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public'
    AS $$
BEGIN
  ...
  WITH session_stats AS (
    -- Calcula tempos e limites da sessão
    SELECT 
      w.id as w_id,
    ...
    FROM workouts w
    JOIN sets s ON s.workout_id = w.id -- Assumindo join via exercise ou direto se tiver coluna
    WHERE w.user_id = p_student_id
    GROUP BY w.id
  ),
  exercise_data AS (
    -- Dados detalhados por exercício
    SELECT 
      e.workout_id,
      e.id as exercise_id,
      ...
      jsonb_agg(jsonb_build_object(
        ...
        'weight', s.weight,
        'reps', s.reps,
        'volume_load', calculate_volume_load(s.weight, s.reps),
        'is_pr', false, -- Lógica de PR seria mais complexa, simplificado aqui
        'rest_seconds', 0 -- Calcular via LAG na aplicação ou em subquery complexa
      ) ORDER BY s.set_number) as sets_data,
      -- Totais do exercício
      SUM(calculate_volume_load(s.weight, s.reps)) as total_vol,
      MAX(s.reps) as max_reps
    FROM exercises e
    JOIN exercise_definitions d ON e.definition_id = d.id
    JOIN sets s ON s.exercise_id = e.id
    GROUP BY e.workout_id, e.id, d.name, e.order_in_workout
  )
  SELECT 
    w.id,
    COALESCE(w.name, 'Treino sem nome'),
    w.workout_date,
    COALESCE(w.coach_read, false), -- Assumindo que criamos essa coluna
    ss.ex_count,
    ss.set_count,
    EXTRACT(EPOCH FROM (ss.end_time - ss.start_time))::integer,
    120, -- Placeholder para média de descanso (requer window functions pesadas)
    jsonb_agg(ed.sets_data ORDER BY ed.order_in_workout)
  FROM workouts w
  JOIN session_stats ss ON ss.w_id = w.id
  JOIN exercise_data ed ON ed.workout_id = w.id
  WHERE w.user_id = p_student_id
  AND (p_filter_unread IS FALSE OR COALESCE(w.coach_read, false) IS FALSE)
  GROUP BY w.id, w.name, w.workout_date, w.coach_read, ss.ex_count, ss.set_count, ss.start_time, ss.end_time
...
ALTER FUNCTION "public"."get_student_diagnostic_feed"("p_student_id" "uuid", "p_limit" integer, "p_offset" integer, "p_filter_unread" boolean) OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."handle_new_user"() RETURNS "trigger"
    ...
    AS $$
BEGIN
  INSERT INTO public.profiles (id, email, display_name, cpf)
  VALUES (
    new.id,
    new.email,
    -- Usa o nome vindo do metadata (Google/Apple/Form) ou parte do email
    COALESCE(new.raw_user_meta_data->>'display_name', new.raw_user_meta_data->>'full_name', split_part(new.email, '@', 1)),
    -- CPF agora é opcional. Se não vier, insere NULL.
...
CREATE OR REPLACE FUNCTION "public"."sanitize_stale_workouts"() RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
  v_user_id uuid := auth.uid();
BEGIN
  -- 1. CASO PRINCIPAL: Fecha treinos antigos que ficaram "abertos"
  -- Define o 'ended_at' como o horário da última série realizada + 1 minuto (para garantir consistência)
  -- A condição w.workout_date < CURRENT_DATE garante que só mexe em treinos de ONTEM para trás.
  UPDATE public.workouts w
  SET ended_at = (
    SELECT COALESCE(MAX(s.performed_at), w.created_at + interval '1 hour')
    FROM public.sets s
    JOIN public.exercises e ON s.exercise_id = e.id
    WHERE e.workout_id = w.id
  )
  WHERE w.user_id = v_user_id
    AND w.workout_date < CURRENT_DATE -- Só treinos passados
    AND w.ended_at IS NULL
    AND EXISTS (
      -- Só finaliza se tiver pelo menos uma série registrada
      SELECT 1 
  ...
  -- 2. LIMPEZA: Remove treinos antigos que estão VAZIOS (Lixo)
  -- Se o treino é velho (ontem ou antes), não foi finalizado e não tem séries, deleta.
...
ALTER FUNCTION "public"."sanitize_stale_workouts"() OWNER TO "postgres";
...
CREATE OR REPLACE TRIGGER "auto-classify-exercise" AFTER INSERT ON "public"."exercise_definitions" FOR EACH ROW EXECUTE FUNCTION "supabase_functions"."http_request"('https://ojkfzowzuyyxgmmljbsc.supabase.co/functions/v1/classify-exercise', 'POST', '{"Content-type":"application/json","Authorization":"Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9qa2Z6b3d6dXl5eGdtbWxqYnNjIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc2MTY4MjY2MSwiZXhwIjoyMDc3MjU4NjYxfQ.aHxf7GO8x2-nffWOEWF_eMc865Z-oSQ6jDKrL2V8Xqo"}', '{}', '5000');
...
ALTER TABLE ONLY "public"."coaching_relationships"
    ADD CONSTRAINT "coaching_relationships_student_id_fkey" FOREIGN KEY ("student_id") REFERENCES "public"."profiles"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."exercises"
    ADD CONSTRAINT "exercises_original_exercise_id_fkey" FOREIGN KEY ("original_exercise_id") REFERENCES "public"."exercises"("id");



ALTER TABLE ONLY "public"."exercises"
    ADD CONSTRAINT "exercises_substituted_by_id_fkey" FOREIGN KEY ("substituted_by_id") REFERENCES "public"."exercises"("id");
...
ALTER TABLE ONLY "public"."workout_sessions"
    ADD CONSTRAINT "workout_sessions_student_id_fkey" FOREIGN KEY ("student_id") REFERENCES "public"."profiles"("id");
...
CREATE POLICY "Limitar programas free" ON "public"."programs" FOR INSERT WITH CHECK (((( SELECT "profiles"."subscription_plan"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = "auth"."uid"())) = 'premium'::"text") OR (( SELECT "count"(*) AS "count"
   FROM "public"."programs" "programs_1"
  WHERE ("programs_1"."student_id" = "auth"."uid"())) < 1)));
...
CREATE POLICY "Permitir leitura para autenticados" ON "public"."coaching_relationships" FOR SELECT USING (("auth"."role"() = 'authenticated'::"text"));
...
CREATE POLICY "Qualquer usuário autenticado pode ver perfis básicos" ON "public"."profiles" FOR SELECT TO "authenticated" USING (true);
...
GRANT ALL ON FUNCTION "public"."calculate_e1rm"("w" numeric, "r" integer, "rpe" numeric) TO "anon";
GRANT ALL ON FUNCTION "public"."calculate_e1rm"("w" numeric, "r" integer, "rpe" numeric) TO "authenticated";
GRANT ALL ON FUNCTION "public"."calculate_e1rm"("w" numeric, "r" integer, "rpe" numeric) TO "service_role";



GRANT ALL ON FUNCTION "public"."calculate_volume_load"("weight" numeric, "reps" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."calculate_volume_load"("weight" numeric, "reps" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."calculate_volume_load"("weight" numeric, "reps" integer) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_exercise_analytics_v2"("p_definition_id" "uuid", "p_target_user_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_exercise_analytics_v2"("p_definition_id" "uuid", "p_target_user_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_exercise_analytics_v2"("p_definition_id" "uuid", "p_target_user_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_student_diagnostic_feed"("p_student_id" "uuid", "p_limit" integer, "p_offset" integer, "p_filter_unread" boolean) TO "anon";
GRANT ALL ON FUNCTION "public"."get_student_diagnostic_feed"("p_student_id" "uuid", "p_limit" integer, "p_offset" integer, "p_filter_unread" boolean) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_student_diagnostic_feed"("p_student_id" "uuid", "p_limit" integer, "p_offset" integer, "p_filter_unread" boolean) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."sanitize_stale_workouts"() TO "anon";
GRANT ALL ON FUNCTION "public"."sanitize_stale_workouts"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."sanitize_stale_workouts"() TO "service_role";
